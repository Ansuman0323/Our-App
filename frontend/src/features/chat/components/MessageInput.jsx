...
export const MessageInput = ({
    ...
    return (
        <div
            ref={containerRef}
            className="bg-white/80 backdrop-blur-md px-4 py-3 md:px-6 md:py-4 border-t border-slate-200 z-10 shrink-0 relative"
        ...
        >
            {isEditing && (
                <div className="max-w-4xl mx-auto flex items-center justify-between gap-3 px-3.5 py-2 mb-2 bg-indigo-50 border border-indigo-100 rounded-xl animate-in fade-in slide-in-from-bottom-1 duration-200">
                    ...
                    <button onClick={handleCancelEdit} type="button" className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 px-2 py-1 rounded-md hover:bg-indigo-100 transition-colors shrink-0">
                        Cancel
                    </button>
                </div>
            )}
            ...
            {replyingToMessage && !isEditing && (
                <div className="max-w-4xl mx-auto flex items-center justify-between gap-3 px-3.5 py-2 mb-2 bg-slate-50 border border-slate-200 rounded-xl animate-in fade-in slide-in-from-bottom-1 duration-200 shadow-sm">
                    ...
                    <button onClick={onCancelReply} type="button" className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full transition-colors shrink-0 focus:outline-none">
            ...
            <form onSubmit={handleSubmit} className="relative flex items-end gap-2 md:gap-3 max-w-4xl mx-auto">
                <div className="flex-1 flex items-end bg-slate-100 rounded-[1.75rem] px-2 py-1.5 border border-transparent focus-within:border-indigo-200 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all duration-200 shadow-inner">
                    ...
                    <div className="flex items-center gap-0.5 pl-1 mb-0.5">
                        <button
                            type="button"
                            onClick={() => setShowPicker(!showPicker)}
                            className={`p-2 rounded-full flex-shrink-0 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${showPicker ? 'bg-indigo-100 text-indigo-600' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-200'}`}
                        ...
                        <button
                            ...
                            className="p-2 rounded-full flex-shrink-0 transition-colors focus:outline-none text-slate-400 hover:text-slate-600 hover:bg-slate-200"
                    ...
                    <textarea
                        ...
                        className="w-full bg-transparent border-none resize-none focus:ring-0 px-2.5 py-2 text-[15px] leading-relaxed text-slate-800 placeholder:text-slate-400 outline-none max-h-32 overflow-y-auto"
                        autoComplete="off"
