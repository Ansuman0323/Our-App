...
import { ImageMessage, VideoMessage, DocumentMessage } from './MediaRenderers';
...
export const MessageBubble = React.memo(({ message, isMine, isConsecutive, onRetry, onOpenActions, onQuoteClick, user, onToggleReaction }) => {
        ...
        <div className={`flex w-full ${isMine ? 'justify-end' : 'justify-start'} ${isConsecutive ? 'mb-1' : 'mb-3 mt-2'} group animate-in slide-in-from-bottom-2 fade-in duration-300 relative`}>
            ...
            >
                <div
                    ...
                    className={`
                        px-3.5 py-2 md:px-4 md:py-2.5 relative transition-all duration-300
                        ${bubbleStyle}
                ...
                >
                    {message.reply && (
                        <div
                            ...
                            role="button"
                            tabIndex={0}
                            className={`mb-1.5 rounded-lg p-2 border-l-4 text-left flex flex-col justify-center transition-colors duration-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-white/50 ${isDeleted
                    ...
                    {/* UPDATED: Render Attachment Layout using the singular attachment object */}
                    {!isDeleted && message.attachment && (
                        <div className={`w-full ${message.content ? 'mb-2' : ''}`}>
                            {message.type === 'IMAGE' && <ImageMessage attachment={message.attachment} isDeleted={isDeleted} />}
                            {message.type === 'VIDEO' && <VideoMessage attachment={message.attachment} isDeleted={isDeleted} />}
                            {message.type === 'FILE' && <DocumentMessage attachment={message.attachment} isDeleted={isDeleted} />}
                        </div>
                    )}

                    <div className={`text-[15px] leading-relaxed break-words whitespace-pre-wrap select-text cursor-auto ${isDeleted ? 'italic flex items-center gap-1.5' : ''}`}>
                        {isDeleted ? (
                            <>
                        ...
                        <span className={`float-right inline-flex items-center gap-1 ml-3 mt-2 text-[10px] md:text-[11px] font-medium transition-colors duration-300 ${isDeleted ? 'text-slate-400' : (isMine ? 'text-indigo-200' : 'text-slate-400')}`}>
                            ...
                            {/* STATUS & PROGRESS */}
                            {!isDeleted && isMine && message.uploadProgress !== undefined && isPending ? (
                                ...
                                <svg className={`w-4 h-4 ${isRead ? 'text-sky-300' : 'text-indigo-200'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 6 8.5 17 4 12.5"></polyline><polyline points="22 6 12.5 17 11 15.3"></polyline></svg>
                            ) : !isDeleted && isMine && !isPending && !isError && !isRead && !isDelivered ? (
                    ...
                    {!isDeleted && groupedReactions.length > 0 && (
                        <div className={`flex flex-wrap gap-1 mt-3 -mb-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
